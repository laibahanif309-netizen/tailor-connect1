...
/**
 * Customer chat — conversation list; tap to open thread.
 */
export default function ChatScreen() {
  const t = useAppScreenTheme();
  const styles = useMemo(() => buildCustomerChatStyles(t), [t]);
  ...
  const handleConversationPress = useCallback((conversationId: string, title: string) => {
    router.push(
      `/(customer)/chat/${conversationId}?title=${encodeURIComponent(title)}`
    );
  }, []);
        ...
        {conversations.length === 0 ? (
          <View style={styles.emptyWrapper}>
            <EmptyState
              ...
              subtitle="Orders and messages with your tailors appear here"
            />
          </View>
        ) : (
          <FlatList
...
function buildCustomerChatStyles(t: AppScreenTheme) {
